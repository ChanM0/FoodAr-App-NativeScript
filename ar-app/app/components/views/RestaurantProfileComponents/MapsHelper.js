...
const MapsUIHelper = {
  ...
  methods: {
    ...
    setMarker(marker, lat, lng, direction) {
      ...
      if (direction !== undefined) marker.rotation = direction;
    }
  }
};
...
const DirectionsAPIHelper = {
  ...
  methods: {
    async hitDirectionsAPI() {
      ...
      // hard code
      APIURL += `?origin=34.235512,-118.531723`;
      // Not hard code
      // APIURL += `?origin=${this.origin.latitude},${this.origin.longitude}`;
      APIURL += `&destination=${this.destination.latitude},${
...
const DistanceMatrixAPIHelper = {
  ...
  methods: {
    getDistance() {
      let distanceMatrixAPIURL =
        "https://maps.googleapis.com/maps/api/distancematrix/json?units=metric&";
      distanceMatrixAPIURL += `origins=${this.origin.latitude},${
        this.origin.longitude
      }&destinations=${this.destination.latitude},${
        this.destination.longitude
      }&key=${this.APIKEY}`;
  ...
  methods: {
    fetchMyLocation() {
      geolocation
        ...
        .then(res => {
          // hard code
          this.origin.latitude = 34.235512;
          this.origin.longitude = -118.531723;
          // not hard code
